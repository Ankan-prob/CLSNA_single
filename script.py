...
ev=3
its=2
rf = "testv2" #rf is the rfilebase parameter

#Generate the reference file
cp = sm.CoupledParticle(N=N,SS=SS,t0=t0,noi=noi,n=10,T=T,gam=gam,d=d,C=C,p=p,a=a\
                        ,delt=delt,dist=dist,iv=iv,ev=ev,its=its)
Ref = sl.SimulateSaveLoad(rf, cp = cp, op = 0,q = q, nl = nl , m=m)
Ref.saveref()
...
for n in ns:
    ...
    Present = sl.SimulateSaveLoad(rf+str(n),rfilebase=rf,cp=cp,op=0,q=q,nl=nl,m=m)
